...
(function ($) {
				...
				if (opts.month == day[1]) {
					...
					if (opts.lunarDay) {
						...
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<span color="#1e9fff">' + info.solarTerms + '</span>';
						} else {
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} '
		 ...
		 + '.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 10; background-color: #1e9fff; display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:150px;}'
